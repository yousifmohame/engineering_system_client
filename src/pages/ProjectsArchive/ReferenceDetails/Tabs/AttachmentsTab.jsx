...
import { 
  ...
  Check, 
  X,
  UploadCloud // 👈 استيراد أيقونة الرفع
} from "lucide-react";
...
// 👈 أضفنا onUploadFile كـ Prop جديد
export default function AttachmentsTab({ data, onDeleteFile, onRenameFile, onUploadFile }) {
  const [editingFileId, setEditingFileId] = useState(null);
  const [newName, setNewName] = useState("");
  
  // 👈 مرجع (Ref) للوصول إلى عنصر اختيار الملفات المخفي
  const fileInputRef = useRef(null);

  // دالة فك التشفير للأسماء العربية
  ...
  const handleFileChange = (e) => {
    ...
    if (file && onUploadFile) {
      onUploadFile(file); // إرسال الملف إلى الدالة الأب
    }
    // تصفير الحقل ليتمكن المستخدم من رفع نفس الملف مرة أخرى إذا أراد
    e.target.value = null; 
  };
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
      ...
      {/* 👈 تعديل رأس التاب ليحتوي على زر الرفع */}
      <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-5">
        <h4 className="text-sm font-black text-slate-800 flex items-center gap-2">
          <FolderArchive className="w-4 h-4 text-indigo-600" /> المصادر والمرفقات ({data.files?.length || 0})
        </h4>
        ...
        {data.files?.map((file, idx) => {
          ...
          return (
            ...
            >
              <div className="flex items-start gap-4 flex-1 overflow-hidden">
                ...
                <div className="overflow-hidden flex-1">
                  {isEditing ? (
                    ...
                    <span className="text-sm font-black text-slate-700 truncate block mt-1" dir="rtl" title={getArabicFileName(file.originalName)}>
              ...
              {/* 👈 أزرار الإجراءات (دائماً ظاهرة ومصممة بألوان هادئة) */}
              {!isEditing && (
