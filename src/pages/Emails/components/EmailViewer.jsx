...
export default function EmailViewer({
  ...
  // 💡 التحميل الكسول: جلب المحتوى الكامل للرسالة بمجرد فتحها
  useEffect(() => {
    let isMounted = true;
    const fetchFullMessage = async () => {
      ...
      try {
        const msgId =
          ...
          selectedMessage.uid;
        const res = await api.get(`/email/messages/${msgId}`);
        if (isMounted) {
          setFullMessage(res.data.data);
          // تحديث حالة "مقروءة" في الخلفية إن لزم الأمر
          if (!selectedMessage.isRead) {
            updateMessageInDB(selectedMessage, { isRead: true });
          }
        }
      } catch (error) {
        if (isMounted) toast.error("تعذر تحميل محتوى الرسالة بالكامل");
      } finally {
  ...
  }, [selectedMessage.id, selectedMessage.messageId, selectedMessage.uid]);

  // نستخدم البيانات الكاملة إن وُجدت، وإلا نستخدم الأساسية مؤقتاً
  const activeMsg = fullMessage || selectedMessage;

  const hasAttachments =
    activeMsg.attachments && activeMsg.attachments.length > 0;

  // نحتفظ بـ isUnread من selectedMessage لمنع الوميض البصري
  ...
  const getRawEmailContent = () => {
    return activeMsg.html || activeMsg.body || activeMsg.text || "";
  };
  ...
  const senderName =
    activeMsg.from?.split("<")[0].replace(/"/g, "").trim() || "بدون مرسل";

  const senderEmail = activeMsg.from?.match(/<([^>]+)>/)?.[1] || activeMsg.from;
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex shrink-0 flex-wrap items-center justify-end gap-1.5">
            <HeaderActionButton
              ...
              title="رد"
              tone="gold"
              onClick={() => handleCompose("reply", activeMsg)}
            >
            ...
            <HeaderActionButton
              ...
              title="رد للجميع"
              tone="gold"
              onClick={() => handleCompose("replyall", activeMsg)}
            >
            ...
            <HeaderActionButton
              ...
              title="تحويل"
              tone="primary"
              onClick={() => handleCompose("forward", activeMsg)}
            >
          ...
          >
            {!activeMsg.isAnalyzed ? (
              <div className="flex h-full flex-col items-center justify-center bg-gradient-to-br from-white via-[#f8efe0] to-cyan-50 p-4 text-center">
                ...
                <button
                  onClick={() => analyzeMutation.mutate(activeMsg)}
                  disabled={analyzeMutation.isPending || isLoadingContent}
              ...
              <div className="flex h-full min-h-0 flex-col overflow-hidden">
                ...
                <div className="min-h-0 flex-1 overflow-y-auto bg-gradient-to-br from-[#fbf8f1] via-white to-cyan-50 p-3 xl:p-4 custom-scrollbar-slim">
                  <div className="grid grid-cols-2 gap-2 xl:hidden">
                    <InfoCard
                      label="نوع الخدمة"
                      value={activeMsg.serviceType}
                      tone="emerald"
                    />

                    <InfoCard
                      label="الجهة المصدرة"
                      value={activeMsg.entityName}
                      icon={Building2}
                      tone="blue"
                    />
                    ...
                    <InfoCard
                      label="سنة الطلب"
                      value={activeMsg.reqYear}
                      tone="cyan"
                    />

                    <InfoCard
                      label="رقم الخدمة"
                      value={activeMsg.serviceNumber}
                      icon={FileText}
                      tone="emerald"
                    />
                  ...
                  <div className="hidden grid-cols-1 gap-3 xl:grid">
                    ...
                    <InfoCard
                      label="القطاع"
                      value={activeMsg.sectorName}
                      tone="gold"
                    />

                    <InfoCard
                      label="وقت الإطلاع"
                      value={activeMsg.viewTime}
                      icon={Clock}
                      tone="red"
                    />
          ...
          >
            ...
            >
              <div className="mb-2 flex items-start justify-between gap-3 xl:mb-3">
                ...
                >
                  {new Date(activeMsg.date).toLocaleString("ar-SA", {
                    dateStyle: "medium",
                    timeStyle: "short",
                  })}
                </div>
              ...
              >
                <div className="flex items-center gap-2.5 xl:gap-3">
                  ...
                  >
                    {activeMsg.from?.charAt(0).toUpperCase() || "?"}
                  </div>
            ...
            {/* EMAIL TEXT - هنا السحر والتحميل الكسول */}
            <div className="min-h-0 flex-1 overflow-y-auto bg-white px-4 py-3 custom-scrollbar-slim xl:px-5 xl:py-4">
              {isLoadingContent ? (
                // 💡 شاشة تحميل وهمية (Skeleton Loader) احترافية
                <div className="animate-pulse space-y-4 py-4">
                  <div className="h-4 w-3/4 rounded-full bg-slate-200/60" />
                ...
                <div className="email-viewer-content">
                  ...
                  {(activeMsg.signature || activeMsg.footerText) &&
                    !isEmailHtmlContent() && (
                          ...
                          <div
                            ...
                            dangerouslySetInnerHTML={{
                              __html: cleanEmailHtml(
                                decodeHtmlEntities(activeMsg.signature),
                              ),
                            }}
                          />
                        )}

                        {activeMsg.footerText && (
                          <div
                            style={{
                              color: activeMsg.footerColor || "#334155",
                              fontSize: activeMsg.footerSize || "11px",
                              textAlign: "center",
                              marginTop: "8px",
                              fontWeight: 700,
                            }}
            ...
            {/* Attachments */}
            {hasAttachments && !isLoadingContent && (
              <div className="shrink-0 border-t border-[#0e7490]/30 bg-gradient-to-l from-[#f8efe0] to-cyan-50 px-4 py-2 xl:px-5 xl:py-3">
                <h4 className="mb-1.5 flex items-center gap-2 text-xs font-black text-[#0f3448] xl:mb-2">
                  ...
                  المرفقات ({activeMsg.attachments.length})
                </h4>

                <div className="flex gap-2 overflow-x-auto pb-1 custom-scrollbar-slim">
                  {activeMsg.attachments.map((att, idx) => (
                    <div
                      key={idx}
...
const HeaderActionButton = ({
  ...
  const tones = {
    gold: "border-[#c5983c]/35 bg-white text-[#0f3448] hover:bg-[#f8efe0] hover:text-[#c5983c]",
    cyan: "border-cyan-700/25 bg-cyan-50 text-cyan-800 hover:bg-cyan-100",
    rose: "border-rose-300 bg-rose-50 text-rose-600 hover:bg-rose-100 hover:text-rose-700",
    emerald:
