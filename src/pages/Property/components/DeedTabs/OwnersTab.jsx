    ...
    <div className="animate-in fade-in max-w-7xl mx-auto">
      <div className="flex items-center justify-between gap-2 mb-4 pb-2 border-b border-[#d8e6ee]">
        ...
        <button
          ...
          className="bg-amber-600 text-white px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-1.5 hover:bg-amber-700"
        >
      ...
      {showOwnerForm && (
        <div className="bg-amber-50 border border-amber-200 p-4 rounded-xl grid grid-cols-4 items-end gap-3 mb-4 animate-in slide-in-from-top-2">
          <div className="col-span-2">
            ...
            <input
              type="text"
              className="w-full p-2.5 text-xs border rounded-xl outline-none focus:border-amber-500"
              value={newOwner.name}
              onChange={(e) =>
          ...
          <button
            ...
            className="px-4 py-2.5 bg-amber-600 text-white text-xs font-bold rounded-xl hover:bg-amber-700"
          >
            إضافة مؤقتة
          </button>
        </div>
      ...
      {localData.owners.length > 0 ? (
        <div className="rounded-xl border border-[#d8e6ee] overflow-hidden bg-white shadow-sm">
          <table className="w-full text-right text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {localData.owners.map((owner, idx) => (
                ...
                >
                  <td className="p-4 font-bold text-[#123B5D] flex items-center gap-2">
                    <div
                      className={`p-2 rounded-full ${idx === 0 ? "bg-amber-100 text-amber-600" : "bg-[#f7fbfd] text-[#71839a]"}`}
                    >
                      <Crown className="w-4 h-4" />
                    </div>
                    {owner.name}
                  </td>
                  <td className="p-4 font-mono text-[#52677e]">
                    {owner.idNumber || owner.identityNumber || "---"}
                  </td>
                  <td className="p-4 font-mono font-black text-[#123B5D] text-lg">
        ...
        <div className="text-center py-16 bg-[#f7fbfd] border-2 border-dashed border-slate-300 rounded-xl">
          ...
          <p className="text-[#71839a] font-bold">لا يوجد ملاك مضافين</p>
        </div>
      )}
    </div>
  );
