...
import { 
  Search, FileText, UploadCloud, FolderOpen, 
  Trash2, Eye, Download, FileCheck, ShieldAlert,
  User, CheckCircle2, AlertCircle, FileImage, 
  FileBox, File, Clock,X
} from "lucide-react";
import { toast } from "sonner";
import api from "../../../../api/axios";

// استيراد هوك المصادقة ومودل عرض الملفات
import { useAuth } from "../../../../context/AuthContext";
import FileViewerModal from "../../../FilesExplorer/modals/FileViewerModal";

// --- دوال مساعدة لاستخراج الأيقونات والألوان حسب نوع الملف ---
const getFileMeta = (fileName) => {
  const ext = fileName?.split('.').pop()?.toLowerCase() || '';
  if (['png', 'jpg', 'jpeg', 'webp', 'gif', 'svg'].includes(ext)) {
    return { icon: FileImage, color: "text-blue-500", bg: "bg-blue-50", label: "صورة" };
  }
  if (ext === 'pdf') {
    return { icon: FileText, color: "text-rose-500", bg: "bg-rose-50", label: "PDF" };
  }
  if (['zip', 'rar', '7z'].includes(ext)) {
    return { icon: FileBox, color: "text-amber-500", bg: "bg-amber-50", label: "مضغوط" };
  }
  return { icon: File, color: "text-gray-500", bg: "bg-gray-50", label: ext.toUpperCase() || "ملف" };
};
...
const ATTACHMENT_CATEGORIES = [
  { id: "CONTRACT", label: "عقود واتفاقيات", icon: FileText, color: "text-blue-600", bg: "bg-blue-50" },
  { id: "ID_PASSPORT", label: "هويات وجوازات", icon: ShieldAlert, color: "text-emerald-600", bg: "bg-emerald-50" },
  { id: "CERTIFICATE", label: "مؤهلات وشهادات", icon: FileCheck, color: "text-purple-600", bg: "bg-purple-50" },
  { id: "OTHER", label: "مرفقات أخرى", icon: FolderOpen, color: "text-gray-600", bg: "bg-gray-50" },
];
...
export default function ContractsAttachmentsEngine() {
  ...
  const [activeFilter, setActiveFilter] = useState("ALL"); // فلتر سريع للتصنيفات
  ...
  const filteredEmployees = useMemo(() => {
    return employees.filter(emp => 
      emp.name?.includes(searchTerm) || emp.employeeCode?.toString().includes(searchTerm)
    );
  }, [employees, searchTerm]);

  // جلب مرفقات الموظف المحدد
  ...
  const uploadMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["employeeAttachments", selectedEmpId] });
  ...
  return (
    <div className="flex h-full w-full overflow-hidden bg-[#fbf8f1] font-cairo" dir="rtl">
      ...
      <div className="w-72 shrink-0 border-l border-[#e8ddc8] bg-white flex flex-col z-10 shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-2 bg-[#fdfdfc]">
          {isLoadingEmployees ? (
            <div className="p-4 text-center text-xs font-bold text-gray-500">جاري التحميل...</div>
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                <div className={`h-10 w-10 rounded-xl flex items-center justify-center shrink-0 font-black text-sm ${
                  selectedEmpId === emp.id ? 'bg-white/20 text-white' : 'bg-[#eef7f6] text-[#0e7490]'
                }`}>
                  {emp.name?.charAt(0)}
                </div>
                <div className="text-right min-w-0 flex-1">
                  <p className="text-[12px] font-black truncate">{emp.name}</p>
                  <p className={`text-[10px] font-bold mt-0.5 ${selectedEmpId === emp.id ? 'text-white/80' : 'text-gray-400'}`}>
      ...
      <div className="flex-1 flex flex-col min-w-0 relative bg-[#fbf8f1]">
        ...
        {!selectedEmpId ? (
          <div className="flex-1 flex flex-col items-center justify-center text-gray-400 gap-4 z-10">
            ...
            <p className="text-base font-black text-gray-500">اختر موظفاً من القائمة الجانبية لعرض المرفقات</p>
          </div>
        ) : (
          <>
            {/* هيدر مساحة العمل */}
              ...
              <div>
                <h2 className="text-lg font-black text-[#06111d] flex items-center gap-2">
                  ...
                  {selectedEmp?.status === "active" ? (
                    <span className="bg-emerald-100 text-emerald-700 text-[10px] px-2 py-0.5 rounded-full flex items-center gap-1"><CheckCircle2 className="h-3 w-3"/> نشط</span>
                  ) : (
                    <span className="bg-rose-100 text-rose-700 text-[10px] px-2 py-0.5 rounded-full flex items-center gap-1"><AlertCircle className="h-3 w-3"/> غير نشط</span>
                  )}
                </h2>
            ...
            <div className="flex-1 overflow-y-auto p-6 custom-scrollbar z-10">
              {isLoadingAttachments ? (
                <div className="flex justify-center items-center h-full"><p className="font-bold text-gray-500 animate-pulse">جاري تحميل المرفقات...</p></div>
              ) : attachments.length === 0 ? (
                <div className="text-center py-24 text-gray-400">
                   <FileText className="h-12 w-12 mx-auto mb-3 text-gray-300" />
                   <p className="font-bold">لا يوجد مرفقات مطابقة للبحث أو التصنيف.</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-5">
                  {attachments.map((file) => {
                    const catInfo = ATTACHMENT_CATEGORIES.find(c => c.id === file.category) || ATTACHMENT_CATEGORIES[3];
                    const meta = getFileMeta(file.fileName);
                    const FileTypeIcon = meta.icon;

                    return (
                      <div key={file.id} className="group relative bg-white border border-gray-100 rounded-[20px] p-4 shadow-sm hover:shadow-[0_8px_24px_rgba(0,0,0,0.04)] transition-all duration-300 hover:border-[#e2bf74]/50 flex flex-col h-[200px]">
                        
                        {/* 1. رأس البطاقة (التصنيف والأيقونة) */}
                        ...
                        <div className="min-w-0 flex-1">
                          <h4 className="text-[13px] font-black text-[#06111d] truncate mb-1" title={file.fileName}>{file.fileName}</h4>
                          <div className="flex items-center gap-2 text-[10px] font-bold text-gray-400">
                            <span className="bg-gray-100 px-1.5 py-0.5 rounded text-gray-600">{meta.label}</span>
                            <span>{formatBytes(file.fileSize)}</span>
                          </div>
                        </div>

                        {/* 3. من قام بالرفع والإجراءات */}
                        <div className="pt-3 mt-auto border-t border-gray-100 flex items-center justify-between">
                          <div className="flex items-center gap-2 max-w-[50%]">
                            <div className="h-6 w-6 rounded-full bg-[#123f59] flex items-center justify-center text-white text-[9px] font-black shrink-0">
                              {file.uploadedBy?.name?.charAt(0) || <User className="h-3 w-3"/>}
                            </div>
                            <div className="truncate">
                              <p className="text-[9px] font-black text-gray-700 truncate">{file.uploadedBy?.name || 'مستخدم النظام'}</p>
                              <p className="text-[8px] font-bold text-gray-400 flex items-center gap-0.5">
                                <Clock className="h-2.5 w-2.5"/> {new Date(file.createdAt).toLocaleDateString("ar-SA")}
                              </p>
                            </div>
                          </div>

                          <div className="flex items-center gap-1.5">
                            {/* 🚀 تجهيز البيانات وتمريرها للمودل */}
                            <button 
                              onClick={() => setFileToView({
                                ...file, // دمج البيانات الأصلية
                                url: file.filePath, // هذا ما يتوقعه FileViewerModal
                                extension: file.fileName?.split('.').pop()?.toLowerCase(), // استخراج الامتداد
                                name: file.fileName, // اسم الملف
                                originalName: file.fileName,
                                size: file.fileSize // حجم الملف
                              })}
                            ...
                            <a 
                              href={file.filePath} // افترضنا أن الرابط مخزن هنا
                              download
                              target="_blank"
                              rel="noreferrer"
                            ...
                            <button 
                              onClick={() => {
                                if(window.confirm("هل أنت متأكد من حذف هذا المرفق نهائياً؟")) {
      ...
      {/* ─── نافذة الرفع (نفسها مع تحسين التنسيق) ─── */}
      {uploadModalOpen && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-[#06111d]/60 backdrop-blur-sm p-4">
          <form onSubmit={handleUpload} className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95">
            <div className="p-5 border-b border-gray-100 bg-gradient-to-br from-gray-50 to-white flex justify-between items-center">
              <div>
                <h3 className="font-black text-[15px] text-[#06111d]">إضافة مستند جديد</h3>
                <p className="text-[11px] text-gray-500 font-bold mt-1">يتم الرفع إلى ملف: {selectedEmp?.name}</p>
              </div>
              <button type="button" onClick={() => setUploadModalOpen(false)} className="h-8 w-8 rounded-full bg-gray-100 text-gray-500 hover:bg-rose-100 hover:text-rose-600 flex items-center justify-center transition"><X className="h-4 w-4"/></button>
            </div>
            <div className="p-5 space-y-5">
              <div>
                <label className="block text-xs font-black text-gray-700 mb-2">تصنيف المستند</label>
              ...
              <div>
                <label className="block text-xs font-black text-gray-700 mb-2">الملف المرفق</label>
            ...
            <div className="p-5 bg-gray-50 border-t border-gray-100 flex justify-end gap-3">
              <button type="button" onClick={() => setUploadModalOpen(false)} className="px-5 py-2.5 rounded-xl text-[12px] font-black text-gray-600 bg-white border border-gray-200 hover:bg-gray-100 transition">إلغاء</button>
              <button type="submit" disabled={uploadMutation.isPending} className="px-6 py-2.5 rounded-xl text-[12px] font-black bg-[#0e7490] text-white hover:bg-[#0b5b73] shadow-[0_4px_12px_rgba(14,116,144,0.25)] flex items-center gap-2 transition disabled:opacity-70">
                {uploadMutation.isPending ? "جاري الرفع..." : <><UploadCloud className="h-4 w-4"/> حفظ المرفق</>}
              </button>
            </div>
          </form>
        </div>
      ...
      {/* ─── نافذة عرض الملفات (يجب أن تعمل الآن بكفاءة) ─── */}
      {fileToView && (
        <FileViewerModal 
          file={fileToView} // يحتوي الآن على url, extension, size, name
