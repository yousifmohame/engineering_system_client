...
export const LivePreview = ({ data }) => {
  ...
  const getMethodIcon = (methodId) => {
    if(methodId === 'bank') return <IconWithText icon={Landmark} iconClassName="w-3 h-3 text-blue-800" />;
    if(methodId === 'cash') return <Banknote className="w-3 h-3 text-[#0f766e]" />;
    if(methodId === 'sadad') return <Wallet className="w-3 h-3 text-orange-600" />;
    return null;
  };

  return (
    <div className="hidden lg:flex w-[46%] border-r border-slate-300 bg-slate-200/80 flex-col h-full relative">
      
      {/* ========================================== */}
      {/* شريط الأدوات الثابت (Toolbar) */}
      {/* ========================================== */}
      <div className="shrink-0 z-50 w-full flex min-w-0 justify-between items-center bg-white/90 backdrop-blur-sm px-4 py-3 shadow-[0_6px_18px_rgba(18,63,89,0.05)] border-b border-[#d8b46a]/25">
        <div className="flex min-w-0 items-center gap-3">
          <div className="p-2 bg-[#123f59] rounded-lg text-white">
            <Eye className="w-5 h-5" />
          </div>
          <div>
            <h3 className="text-xs font-black text-[#123f59] uppercase tracking-tighter">معاينة الطباعة (A4)</h3>
            <p className="text-[10px] text-[#64748b] font-bold">{templateType === "SUMMARY" ? "مختصر" : "تفصيلي"}</p>
          </div>
        </div>

        <div className="flex min-w-0 items-center gap-2">
          {/* أزرار التكبير والتصغير */}
          <div className="flex min-w-0 items-center bg-slate-100 rounded-xl p-1 border border-[#d8b46a]/25 mr-2">
            <button onClick={handleZoomOut} className="p-1.5 text-[#64748b] hover:text-[#123f59] hover:bg-white rounded-lg transition-all">
              <ZoomOut className="w-4 h-4" />
            </button>
            <span className="text-[10px] font-black text-[#475569] min-w-[35px] text-center">
              {Math.round(zoomScale * 100)}%
            </span>
            <button onClick={handleZoomIn} className="p-1.5 text-[#64748b] hover:text-[#123f59] hover:bg-white rounded-lg transition-all">
          ...
          <button 
            ...
            className={`flex min-w-0 items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-black transition-all ${
              isEditMode 
              ? "bg-emerald-100 text-[#0f766e] border border-emerald-200" 
              : "bg-white text-[#475569] hover:bg-[#fbf8f1] border border-[#d8b46a]/25"
            }`}
          >
          ...
          <button 
            onClick={() => handlePrint()}
            className="flex min-w-0 items-center gap-2 px-4 py-2.5 bg-[#123f59] text-white rounded-xl text-xs font-black hover:bg-[#0f3448] shadow-[0_8px_18px_rgba(18,63,89,0.08)] shadow-blue-200 transition-all active:scale-95"
          >
      ...
      {/* ========================================== */}
      <div className="flex-1 w-full overflow-y-auto overflow-x-hidden flex justify-center items-start pt-8 pb-32 custom-scrollbar-slim">
        ...
        >
          ...
          <div
            ref={componentRef}
            className="bg-white shadow-[0_20px_55px_rgba(18,63,89,0.18)] relative border border-slate-300 print-area"
            style={{
              width: "210mm",
              minHeight: "297mm",
            ...
            <div className="p-[15mm] relative z-10 flex flex-col h-full">
              ...
              <div className="flex min-w-0 justify-between items-start border-b-4 border-blue-900 pb-6 mb-3">
                ...
                <div className="text-left mt-2">
                  <h3 className="text-lg font-black text-blue-900 mb-3 tracking-tighter">عرض سعر خدمات</h3>
                  <div className="text-[10px] font-bold text-[#475569] space-y-1.5">
                    <p className="flex min-w-0 justify-between gap-3"><span>التاريخ:</span> <span>{issueDate}</span></p>
                    <p className="flex min-w-0 justify-between gap-3"><span>المرجع:</span> <span className="font-mono text-[#123f59]">{refNumber}</span></p>
                  </div>
                </div>
              </div>
              ...
              <div className="mb-3">
                <div className="flex items-baseline gap-1 mb-1">
                  <span className="text-[13px] font-black text-[#123f59]">السادة /</span>
                  <div 
                    contentEditable={isEditMode}
                    suppressContentEditableWarning={true}
                    className={`text-[13px] font-black text-[#123f59] flex min-w-0 items-center gap-2 ${isEditMode ? "bg-yellow-50 border-b border-dashed border-yellow-400 outline-none px-2" : ""}`}
                  >
                    {clientTitle !== "لقب مخصص" && clientTitle ? `${clientTitle} ` : ""}{clientNameForPreview || "اسم العميل"}
                    {showClientCode && clientCodeForPreview && (
                      <span className="bg-slate-100 text-[#64748b] px-1.5 py-0.5 rounded text-[9px] font-mono border border-[#d8b46a]/25">
                        {clientCodeForPreview}
                      </span>
                    )}
                  </div>
                </div>
                <p className="text-[13px] font-black text-[#123f59] mb-3">المحترم</p>
                <p className="text-[12px] font-bold text-[#123f59] mb-3">السلام عليكم ورحمة الله وبركاته ،،،،</p>
                
                <div 
                  contentEditable={isEditMode}
                  suppressContentEditableWarning={true}
                  className={`text-[12px] leading-[2] text-[#475569] text-justify ${isEditMode ? "bg-yellow-50 border border-dashed border-yellow-400 outline-none p-3 rounded" : ""}`}
                >
                  {introText}
                </div>
              </div>
                  ...
                  <tbody className="text-[#123f59] font-bold">
                    {items?.map((item, idx) => (
                      <tr key={item.id || idx} className={idx % 2 === 0 ? "bg-white" : "bg-blue-50/30"}>
                        <td className="border border-blue-100 p-2 font-mono text-[#94a3b8]">{idx + 1}</td>
                    ...
                    {(!items || items.length === 0) && (
                      <tr>
                        <td colSpan="6" className="border border-blue-100 p-3 text-center text-[#94a3b8]">
                          لا يوجد بنود مسجلة
                        </td>
                      </tr>
                    )}
                    ...
                    <tr>
                      <td colSpan="5" className="border border-blue-900 p-2 text-left font-bold text-slate-600">
                        ...
                        {officeTaxBearing > 0 && <span className="text-[9px] text-[#0f766e] mr-2">(المكتب يتحمل {officeTaxBearing}% من الضريبة)</span>}
                      </td>
              ...
              {((paymentsList && paymentsList.length > 0) || (acceptedMethods && acceptedMethods.length > 0)) && (
                <div className="mb-3 grid grid-cols-12 gap-3 items-start">
                  {paymentsList && paymentsList.length > 0 && (
                    <div className="col-span-8">
                      <h4 className="text-[12px] font-black text-blue-900 mb-2">جدولة الدفعات :</h4>
                      <table className="w-full border-collapse border border-slate-300 text-[10px] text-center shadow-[0_6px_18px_rgba(18,63,89,0.05)]">
                        <thead className="bg-slate-100 text-[#475569]">
                          <tr>
                        ...
                        <tbody>
                          {paymentsList.map((p) => (
                            <tr key={p.id}>
                              <td className="border border-slate-300 p-1.5 font-bold bg-gradient-to-br from-[#eef7f6] via-[#fbf8f1] to-white">{p.label}</td>
                              ...
                              <td className="border border-slate-300 p-1.5 text-right font-bold text-[#475569]">{p.condition}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                    ...
                    <div className="col-span-4">
                      ...
                      <div className="flex flex-col gap-1.5">
                        {acceptedMethods.map(method => (
                          <div key={method} className="flex min-w-0 items-center gap-1.5 text-[10px] font-bold text-[#475569] bg-gradient-to-br from-[#eef7f6] via-[#fbf8f1] to-white border border-[#d8b46a]/25 px-2 py-1.5 rounded">
              ...
              <div className="mb-auto">
                <div className="flex min-w-0 items-center gap-2 mb-3 border-r-4 border-blue-900 pr-3">
                   <h4 className="text-[13px] font-black text-blue-900 uppercase">الشروط والأحكام :</h4>
                </div>
                <div 
                  contentEditable={isEditMode}
                  suppressContentEditableWarning={true}
                  className={`text-[11px] leading-[1.8] text-[#475569] whitespace-pre-line text-justify font-bold ${isEditMode ? "bg-yellow-50 border border-dashed border-yellow-400 p-3 rounded-lg outline-none" : ""}`}
                >
              ...
              <div className="absolute bottom-0 left-0 right-0 bg-white px-[15mm] pb-[10mm] pt-4">
                ...
                <div className="border-t border-[#d8b46a]/25 pt-6 flex min-w-0 justify-between items-end text-[10px] text-[#64748b] font-[Tajawal] print:border-slate-300 break-inside-avoid">
                  ...
                  <div className="flex flex-col gap-1 w-1/3">
                    <div className="flex min-w-0 items-center gap-2">
                      <span className="font-bold text-[#123f59] tracking-tight">الرقم المرجعي:</span>
                  ...
                  <div className="flex justify-end gap-3 items-end w-1/3">
                    ...
                    <div className="text-left font-black text-[#123f59] border-r-2 border-slate-900 pr-4 h-full flex flex-col justify-center">
                      <div className="text-xs uppercase tracking-widest text-[#94a3b8] mb-0.5">Page</div>
                ...
                <div className="mt-4 pt-4 border-t border-[#d8b46a]/25 flex flex-col sm:flex-row items-center justify-between text-[10px] text-[#64748b] font-bold gap-2">
                  <div className="flex gap-3">
                    <span className="flex min-w-0 items-center gap-1"><span className="text-[#94a3b8]">هاتف:</span> 0547267500</span>
                    <span className="flex min-w-0 items-center gap-1"><span className="text-[#94a3b8]">بريد:</span> info@blackcube.sa</span>
                    <span className="flex min-w-0 items-center gap-1"><span className="text-[#94a3b8]">الموقع:</span> www.blackcube.sa</span>
                  </div>
                  
                  <div className="text-[9px] text-[#94a3b8] max-w-md text-center hidden md:block">
