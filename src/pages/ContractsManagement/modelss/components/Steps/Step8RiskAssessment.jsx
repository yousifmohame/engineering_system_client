...
export const Step8RiskAssessment = ({
  ...
  <div className="space-y-4 animate-in slide-in-from-right-4">
    <div className="p-4 bg-gradient-to-br from-violet-50 to-fuchsia-50 rounded-[22px] border border-violet-100 shadow-sm space-y-4">
      <div className="flex items-center justify-between border-b border-violet-200/50 pb-4">
        <h3 className="text-[13px] font-black text-violet-900 flex items-center gap-2">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-violet-600 text-white text-[11px] font-black rounded-xl hover:bg-violet-700 transition-colors shadow-lg shadow-violet-600/20 disabled:opacity-50"
        >
          {isGeneratingAI ? (
      ...
      <div className="bg-white/60 backdrop-blur-sm p-4 rounded-xl border border-white/50 min-h-[200px]">
        {contract.aiRiskAssessment ? (
          <div className="whitespace-pre-wrap text-sm text-[#123B5D] leading-relaxed font-medium">
            {contract.aiRiskAssessment}
          </div>
        ) : (
