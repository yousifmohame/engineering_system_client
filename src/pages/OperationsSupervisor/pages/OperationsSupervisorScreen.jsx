...
export default function OperationsSupervisorScreen() {
  ...
  const handleReject = async () => {
    ...
    try {
      await api.put(`/documentation/${selectedDoc.id}/reject`, {
        reason: rejectReason,
      });
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-black text-slate-800">
              مشرف العمليات والاعتمادات
            </h2>
            <div className="flex items-center gap-2 mt-1">
              <span className="text-xs font-bold text-slate-500 flex items-center gap-1">
                <User className="w-3.5 h-3.5" /> المشرف الحالي:{" "}
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center h-full text-slate-400 opacity-70">
                ...
                <p className="font-black text-sm text-emerald-600">
                  صندوق الاعتمادات فارغ
                </p>
                <p className="text-xs font-bold mt-1">
                  عمل رائع! تم إنجاز كافة المهام.
                </p>
              </div>
            ) : (
              pendingDocs.map((doc) => (
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    <span className="text-sm font-black text-slate-800 line-clamp-1 pl-2">
                      {doc.name}
                    </span>
                    <span className="text-[9px] font-black text-amber-600 bg-amber-100 px-1.5 py-0.5 rounded border border-amber-200 shrink-0">
                      معلق
                    </span>
                  </div>
                  <div className="flex flex-col gap-1.5 text-[10px] font-bold text-slate-500">
                    ...
                    <div className="flex justify-between items-center mt-1 pt-2 border-t border-slate-100">
                      <span className="flex items-center gap-1 text-slate-400">
                        <User className="w-3 h-3" /> {doc.createdBy || "موظف"}
                      </span>
                      <span className="flex items-center gap-1 text-slate-400">
        ...
        <div className="flex-1 bg-white flex flex-col overflow-y-auto custom-scrollbar">
          {!selectedDoc ? (
            <div className="flex-1 flex flex-col items-center justify-center text-slate-400 bg-slate-50/30">
              ...
              <p className="font-black text-xl text-slate-500">
                اختر مستنداً للمراجعة
              </p>
              <p className="text-sm font-bold mt-2 text-slate-400">
                انقر على أي طلب من القائمة لعرض تفاصيله واعتماده.
              </p>
            </div>
          ) : (
              ...
              <div className="flex-1 p-6 md:p-8 flex flex-col gap-8 max-w-5xl mx-auto w-full">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100">
                    <h4 className="text-sm font-black text-slate-800 flex items-center gap-2 border-b border-slate-200 pb-3 mb-4">
                      <LinkIcon className="w-4 h-4 text-blue-500" /> ارتباطات
                      المستند
                    </h4>
                    <div className="space-y-3 text-xs font-bold">
                      <div className="flex justify-between">
                        <span className="text-slate-500">نوع المستند:</span>{" "}
                        <span className="text-slate-800 bg-white px-2 py-0.5 border rounded">
                          {selectedDoc.type}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        ...
                        <span className="text-slate-800">
                          {selectedDoc.transactionId || "غير مرتبط"}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-500">هوية العميل:</span>{" "}
                        <span className="text-slate-800">
                          {selectedDoc.clientId || "غير مرتبط"}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-500">الطرف الثاني:</span>{" "}
                        <span className="text-slate-800">
                          {selectedDoc.partyB || "غير محدد"}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100">
                    <h4 className="text-sm font-black text-slate-800 flex items-center gap-2 border-b border-slate-200 pb-3 mb-4">
                      <ShieldCheck className="w-4 h-4 text-emerald-500" /> قيود
                      الأمان المطبقة
                    </h4>
                    <div className="space-y-3 text-xs font-bold">
                      <div className="flex justify-between">
                        <span className="text-slate-500">التحقق العام:</span>
                        <span
                          className={
                            selectedDoc.isVerifiable
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-slate-800">
                          {selectedDoc.maxViews
                            ? `${selectedDoc.maxViews} مرات فقط`
                            : "غير محدود ♾️"}
                        </span>
                      </div>
                      <div className="flex justify-between">
                ...
                <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100">
                  <h4 className="text-sm font-black text-slate-800 flex items-center gap-2 border-b border-slate-200 pb-3 mb-4">
                    <History className="w-4 h-4 text-amber-500" /> مسار المستند
                  </h4>
                  <div className="flex items-center gap-8 px-4 overflow-x-auto custom-scrollbar-slim pb-2">
                    ...
                    <div className="flex flex-col items-center shrink-0">
                      <div className="w-8 h-8 rounded-full bg-amber-500 text-white flex items-center justify-center mb-2 animate-pulse shadow-lg shadow-amber-500/30">
                ...
                <div className="max-w-5xl mx-auto">
                  {showRejectInput ? (
                    <motion.div
                      ...
                      className="flex flex-col gap-3 bg-rose-50 p-4 rounded-2xl border border-rose-100"
                    >
                      <label className="text-sm font-black text-rose-800 flex items-center gap-1.5">
                        <XCircle className="w-4 h-4" /> سبب الرفض (إلزامي)
                      </label>
                      <input
                        type="text"
                      ...
                      <div className="flex gap-3 mt-2">
                        <button
                          ...
                          className="flex-1 bg-rose-600 text-white text-sm font-black py-3 rounded-xl hover:bg-rose-700 shadow-md shadow-rose-600/20 disabled:opacity-50 transition-all flex justify-center items-center gap-2"
                        >
                          {isProcessing ? (
                        ...
                        <button
                          ...
                          className="px-8 bg-white border border-slate-300 text-slate-700 text-sm font-black py-3 rounded-xl hover:bg-slate-50 transition-all"
                        >
                          تراجع
                        </button>
                      </div>
