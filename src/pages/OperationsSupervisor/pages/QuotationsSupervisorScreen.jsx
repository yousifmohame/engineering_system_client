...
const translateAction = (action) => {
  switch (action) {
    case "CREATE":
      return { label: "إنشاء مسودة", color: "text-blue-600 bg-blue-50" };
    case "SUBMIT":
      return { label: "إرسال للمراجعة", color: "text-purple-600 bg-purple-50" };
    case "REQUEST_MODIFICATION":
      return { label: "طلب تعديل", color: "text-amber-600 bg-amber-50" };
    case "REJECT":
      return { label: "رفض نهائي", color: "text-rose-600 bg-rose-50" };
    case "APPROVE":
      return { label: "اعتماد وختم", color: "text-emerald-600 bg-emerald-50" };
    case "RESTORE":
      return { label: "استرجاع من السلة", color: "text-cyan-600 bg-cyan-50" };
    default:
      return { label: "إجراء نظام", color: "text-slate-600 bg-slate-50" };
  }
};

// ==========================================
// 1. المكون الفرعي: نافذة المراجعة والاعتماد (للمشرف فقط)
// ==========================================
const QuotationReviewModal = ({ quoteId, onClose }) => {
  ...
  const actionMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      if (variables.actionType === "APPROVE")
        msg = "تم اعتماد العرض وختمه إلكترونياً!";
      if (variables.actionType === "REJECT")
        msg = "تم رفض العرض وإرسال السبب للموظف.";
      if (variables.actionType === "MODIFY")
        msg = "تمت إعادة العرض للموظف لعمل التعديلات المطلوبة.";
  ...
  const handleActionClick = (type) => {
    if (type === "APPROVE") {
      if (
        window.confirm("هل أنت متأكد من اعتماد هذا العرض وختمه بشكل نهائي؟")
      ) {
  ...
  const submitReasonAction = () => {
    if (!reasonModal.text.trim())
      return toast.error("الرجاء كتابة الملاحظات أو السبب أولاً");
    const payload =
      reasonModal.type === "REJECT"
        ? { reason: reasonModal.text }
        : { notes: reasonModal.text };
      ...
      {reasonModal.isOpen && (
        <div className="fixed inset-0 z-[400] flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in zoom-in-95">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden border border-slate-200">
            <div
              className={`p-4 text-white font-black flex items-center gap-2 ${reasonModal.type === "REJECT" ? "bg-rose-600" : "bg-amber-500"}`}
            >
              {reasonModal.type === "REJECT" ? (
                <XCircle size={18} />
              ) : (
              ...
              {reasonModal.type === "REJECT"
                ? "تأكيد الرفض النهائي للعرض"
                : "طلب تعديل من الموظف"}
            </div>
            <div className="p-4">
              <label className="block text-xs font-bold text-slate-700 mb-2">
                {reasonModal.type === "REJECT"
                  ? "الرجاء توضيح سبب الرفض (سيتم حفظه في السجل):"
                  : "الرجاء كتابة التعديلات المطلوبة بوضوح:"}
              </label>
              <textarea
                value={reasonModal.text}
                onChange={(e) =>
                  setReasonModal({ ...reasonModal, text: e.target.value })
                }
              ...
              <div className="flex gap-2 mt-4">
                ...
                <button
                  ...
                  className="px-6 py-2 bg-slate-100 text-slate-600 hover:bg-slate-200 text-sm font-bold rounded-xl transition-colors"
                >
                  إلغاء
                </button>
              </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-white/60 font-bold mt-0.5">
                شاشة التدقيق والمراجعة للمشرفين
              </p>
            </div>
          </div>
          <button
            ...
            <div className="w-[380px] shrink-0 border-l border-slate-200 bg-white flex flex-col shadow-[4px_0_24px_rgba(0,0,0,0.02)] z-10">
              <div className="flex-1 overflow-y-auto p-4 custom-scrollbar-slim space-y-4">
                <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                  ...
                  <div className="space-y-2.5 text-xs font-bold text-[#123f59]">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {quote.ownership?.code || "بدون ملكية مرتبطة"}
                    </div>
                    <div className="flex items-center gap-2">
                      <Landmark className="w-3.5 h-3.5 text-amber-500" /> قالب:{" "}
                      {quote.templateType === "SUMMARY" ? "مختصر" : "تفصيلي"}
                    </div>
                  </div>
                </div>
                ...
                <div className="p-3 bg-gradient-to-br from-[#eef7f6] to-white rounded-xl border border-emerald-100">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex justify-between text-xs text-slate-600">
                      <span>الضريبة ({quote.taxRate * 100}%):</span>{" "}
                      <span className="font-mono font-bold">
                  ...
                  {quote.attachments?.length > 0 ? (
                    <div className="space-y-2 max-h-48 overflow-y-auto custom-scrollbar-slim pr-1">
                      {quote.attachments.map((att) => (
                        <div
                          key={att.id}
                          className="flex items-center justify-between p-2 bg-white rounded-lg border border-blue-100 hover:border-blue-300 transition-all group"
                        >
                          ...
                          <div key={log.id} className="relative">
                            ...
                            <div className="bg-slate-50 rounded-lg p-2 border border-slate-100">
                              <div className="flex justify-between items-start mb-1">
                                <span
                                  className={`px-1.5 py-0.5 rounded text-[9px] font-black ${meta.color}`}
                                >
                                  {meta.label}
                                </span>
              ...
              {quote.status === "PENDING_APPROVAL" ? (
                <div className="p-4 border-t border-slate-200 bg-slate-50 flex flex-col gap-2 shrink-0 shadow-[0_-4px_10px_rgba(0,0,0,0.02)]">
                  ...
                  >
                    {actionMutation.isPending && reasonModal.type === null ? (
              ...
              {quote.pdfUrl ? (
                ...
                <div className="w-full h-full rounded-xl border-2 border-dashed border-slate-300 bg-white flex flex-col items-center justify-center text-slate-400">
                  ...
                  <span className="text-sm font-bold">
                    لم يتم توليد ملف PDF النهائي بعد
                  </span>
                  <span className="text-xs mt-1">
                    سيتم توليد الملف تلقائياً بمجرد الاعتماد وتطبيق الختم
                  </span>
                </div>
              )}
            </div>
...
export default function QuotationsSupervisorScreen() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      // 🚀 التعديل هنا: جلب فقط العروض التي تنتظر المراجعة (PENDING_APPROVAL)
      return res.data.data.filter((q) => q.status === "PENDING_APPROVAL");
    },
      ...
      <div className="flex justify-between items-end mb-4">
        <div>
          ...
          <p className="text-xs font-bold text-slate-500 mt-1">
            إدارة عروض الأسعار التي أرسلها الموظفون وتتطلب مراجعتك أو اعتمادك
            النهائي.
          </p>
        </div>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col h-full items-center justify-center text-slate-400 gap-2">
              ...
              <span className="text-xs">
                لا توجد عروض أسعار بانتظار الاعتماد أو المراجعة حالياً.
              </span>
            </div>
          ) : (
                  ...
                  >
                    ...
                    <td className="p-3 text-center">
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-black transition-colors shadow-sm bg-[#123f59] text-white hover:bg-[#0e7490] animate-pulse-slow"
                      >
