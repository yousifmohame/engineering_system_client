...
import {
  Clock, Plus, Save, User, Check, CalendarDays, Loader2, X, Trash2, Edit3, MessageSquare, Flag
} from "lucide-react";
import { toast } from "sonner";
import api from "../../../../api/axios";
import { useAuth } from "../../../../context/AuthContext";

// 💡 مكون العداد التنازلي (لم يتغير)
const CountdownTimer = ({ targetDate }) => {
  ...
  return (
    <span className={`font-mono font-bold text-xs px-2 py-1 rounded flex items-center gap-1 ${isUrgent ? "bg-red-100 text-red-700" : "bg-blue-100 text-blue-700"}`}>
...
export const TasksTab = ({
  ...
  isSuperAdmin,
  persons,
  addTaskMutation, // 👈 سيتم استخدامها لاحقاً إذا أردت دمج الإضافة هنا
  submitTaskMutation,
  ...
  // 🚀 جلب المهام المرتبطة بهذه المعاملة تحديداً من OfficeTasks
  const { data: transactionTasks = [], isLoading: isTasksLoading } = useQuery({
    queryKey: ["transaction-office-tasks", tx.id],
    queryFn: async () => {
      // نستخدم مسار جلب كل المهام ونفلتر، أو نعتمد على API مخصص للمعاملة
      const res = await api.get("/office-tasks");
      const allTasks = res.data?.data || [];
      // فلترة المهام المرتبطة بـ ID هذه المعاملة
  ...
  const [taskForm, setTaskForm] = useState({
    ...
    dueDate: "",
    priority: "medium",
    assignedEmployees: [], // مصفوفة الموظفين
  });
  ...
  const saveTaskMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries(["office-tasks"]); // لتحديث مفكرة المكتب أيضاً
      setIsAdding(false);
      setTaskForm({ title: `مهمة جديدة - ${tx.transactionCode || 'معاملة'}`, description: "", dueDate: "", priority: "medium", assignedEmployees: [] });
    },
  ...
  const completeTaskMutation = useMutation({
    mutationFn: async (taskId) => api.put(`/office-tasks/${taskId}/status`, { status: "completed" }),
    onSuccess: () => {
    ...
    <div className="space-y-6 animate-in fade-in pb-10">
      ...
      <div className="flex items-center justify-between border-b border-slate-200 pb-3">
        <h3 className="font-bold text-slate-800 flex items-center gap-2">
          <CalendarDays className="w-5 h-5 text-indigo-600" /> مهام المعاملة ({transactionTasks.length})
        </h3>
      ...
      {isAdding && isSuperAdmin && (
        <div className="bg-indigo-50/50 p-5 rounded-xl border border-indigo-100 space-y-4 animate-in slide-in-from-top-2">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-bold text-indigo-800 mb-1 block">عنوان المهمة</label>
              <input
                type="text"
            ...
            <div>
              <label className="text-xs font-bold text-indigo-800 mb-1 block">اختر الموظف المستهدف</label>
              <select
                value={taskForm.assignedEmployees.length > 0 ? taskForm.assignedEmployees[0].name : ""}
                onChange={(e) => {
                  const empName = e.target.value;
                  const emp = employees.find(p => p.name === empName);
                  setTaskForm({ 
                    ...taskForm, 
                    assignedEmployees: emp ? [{ id: emp.id || "auto", name: emp.name }] : [] 
                  });
                }}
                className="w-full border p-2.5 rounded-lg text-sm font-bold outline-none focus:border-indigo-500 bg-white"
              >
            ...
            <div>
              <label className="text-xs font-bold text-indigo-800 mb-1 block">تاريخ التسليم النهائي</label>
              <input
                type="date"
            ...
            <div>
              <label className="text-xs font-bold text-indigo-800 mb-1 block">الأولوية</label>
              <select
            ...
            <div className="col-span-2">
              <label className="text-xs font-bold text-indigo-800 mb-1 block">وصف المهمة بدقة</label>
              <textarea
          ...
          <div className="flex justify-end pt-2">
            ...
            >
              {saveTaskMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
              حفظ وإرسال المهمة
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4">
          {transactionTasks.length > 0 ? (
            transactionTasks.map((t) => {
              ...
              if (!isSuperAdmin && !isMyTask) return null;

              return (
                <div key={t.id} className={`p-5 rounded-2xl border shadow-sm transition-all ${isCompleted ? "bg-slate-50 border-slate-200" : isFrozen || isCancelled ? "bg-slate-100 border-slate-200 opacity-60" : "bg-white border-indigo-100 hover:border-indigo-300"}`}>
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-start gap-3">
                      <div className={`p-2.5 rounded-lg shrink-0 mt-1 ${isCompleted ? "bg-slate-200 text-slate-500" : t.priority === "high" ? "bg-red-100 text-red-600" : "bg-indigo-100 text-indigo-600"}`}>
                        {t.priority === "high" ? <Flag className="w-5 h-5" /> : <User className="w-5 h-5" />}
                      </div>
                      <div className="flex flex-col gap-1">
                        <div className={`font-black text-sm ${isCompleted ? "text-slate-500 line-through" : "text-slate-900"}`}>{t.title || "مهمة"}</div>
                        ...
                        <div className="text-[10px] font-bold text-slate-400">
                          بواسطة: {t.creatorName || "النظام"} | {new Date(t.createdAt).toLocaleDateString("ar-SA")}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      {isSuperAdmin && (
                        <div className="flex items-center gap-1 mr-4 bg-slate-50 px-2 py-1 rounded-lg border border-slate-100">
                          <button onClick={() => { if (window.confirm("حذف المهمة نهائياً؟")) deleteTaskMutation.mutate(t.id); }} className="p-1.5 text-red-500 hover:bg-red-100 rounded-md transition-colors" title="حذف المهمة">
                      ...
                      {!isCompleted && !isFrozen && !isCancelled ? (
                        <div className="flex flex-col items-end gap-2">
                           ...
                           {(isMyTask || isSuperAdmin) && (
                              <button onClick={() => { if(window.confirm("تأكيد إتمام المهمة؟")) completeTaskMutation.mutate(t.id) }} className="text-[10px] font-black bg-emerald-100 text-emerald-700 hover:bg-emerald-200 px-3 py-1.5 rounded-lg flex items-center gap-1 transition-colors">
                        ...
                        <span className="bg-slate-100 text-slate-600 px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 border border-slate-200">
                          {isCompleted ? "مكتملة ✅" : isFrozen ? "مجمدة ❄️" : "ملغاة ❌"}
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  {t.comments && t.comments.length > 0 && (
                    <div className="mt-3 pt-3 border-t border-slate-100">
                       <h5 className="text-[10px] font-black text-slate-500 mb-2 flex items-center gap-1"><MessageSquare className="w-3 h-3" /> الملاحظات والتعليقات</h5>
                       <div className="space-y-2">
                         {t.comments.map(c => (
                            <div key={c.id} className="bg-white p-2 rounded-lg border border-slate-100 text-[11px] font-bold text-slate-700">
                               <span className="text-blue-600">{c.authorName}:</span> {c.text}
                            </div>
                         ))}
                       </div>
                    </div>
