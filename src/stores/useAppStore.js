...
export const SCREENS = {
  DASH: { id: "DASH", title: "لوحة التحكم", icon: "LayoutDashboard", closable: false },
...
export const useAppStore = create((set, get) => ({
  ...
  // ==========================================
  screenTabs: {
    DASH: [{ id: "DASH-MAIN", title: "النظرة العامة", type: "dashboard", closable: false }],
    "055": [{ id: "055-MAIN", title: "إدارة المعاملات", type: "wrapper", closable: false }],
    300: [{ id: "DASHBOARD_CLIENTS", title: "لوحة العملاء", type: "dashboard", closable: false }],
    310: [{ id: "DASHBOARD_TAB", title: "لوحة الملكيات", type: "dashboard", closable: false }],
    815: [{ id: "QUOTATIONS_DASH", title: "لوحة عروض الأسعار", type: "dashboard", closable: false }],
    942: [{ id: "942-MAIN", title: "الإعدادات العامة", type: "wrapper", closable: false }],
    SET: [{ id: "SET-SERVER", title: "حالة السيرفر", type: "wrapper", closable: false }],
  },
  ...
  openScreen: (screenId, dynamicTitle = "شاشة جديدة") =>
    set((state) => {
      const screenConfig = SCREENS[screenId] || { id: screenId, title: dynamicTitle };
      ...
      return {
        openScreens: [...state.openScreens, { id: screenId, title: screenConfig.title, isClosable: true }],
        activeScreenId: screenId,
        screenTabs: {
          ...state.screenTabs,
          ...(hasExistingTabs ? {} : { [screenId]: [{ id: mainTabId, title: screenConfig.title, type: "wrapper", closable: false }] }),
        },
        activeTabPerScreen: {
  ...
  closeScreen: (screenId) =>
    set((state) => {
      ...
      if (state.activeScreenId === screenId) {
        newActiveId = newScreens.length > 0 ? newScreens[newScreens.length - 1].id : "DASH";
      }
  ...
  addTab: (screenId, tab) =>
    set((state) => {
      ...
      if (isTabExists) {
        return { activeTabPerScreen: { ...state.activeTabPerScreen, [screenId]: tab.id } };
      }

      const newTab = { ...tab, closable: tab.closable !== false };
      return {
        screenTabs: { ...state.screenTabs, [screenId]: [...existingTabs, newTab] },
  ...
  removeTab: (screenId, tabId) =>
    set((state) => {
      ...
      return {
        ...
        activeTabPerScreen: { ...state.activeTabPerScreen, [screenId]: newActiveTab },
      };
    }),
